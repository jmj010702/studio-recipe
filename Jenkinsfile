pipeline {
    ...
    stages {
        ...
        stage('Rename JAR file and Prepare for Docker') { // 스테이지 이름 변경
            steps {
                script {
                    ...
                    def jarToUse = "" // jarToRename 대신 jarToUse 사용
                    ...
                    if (jarToUse) {
                        // 찾은 JAR 파일을 Docker 빌드 컨텍스트인 recipe/ 디렉토리로 app.jar 이름으로 복사
        ...
        stage('Docker Build & Push to ECR') {
            steps {
                script {
                    ...
                    // Dockerfile은 recipe/app.jar를 복사할 것이므로, build context는 recipe가 되고
                    // COPY app.jar app.jar 로 Dockerfile을 수정합니다.
                    // Jenkinsfile에서 jar 파일을 recipe/app.jar로 미리 복사했기 때문에 추가 cp는 필요 없습니다.

                    sh "docker build -t ${ECR_IMAGE} -f recipe/Dockerfile recipe" // Dockerfile이 recipe/ 안에 있고 빌드 컨텍스트도 recipe
        ...
        stage('Prepare and Deploy to CodeDeploy') {
            steps {
                script {
                    ...
                    // CodeDeploy 번들에는 app.jar을 포함할 필요가 없습니다. Docker 이미지를 실행할 것이기 때문입니다.
                    // app.jar은 이미 Docker 이미지 내에 포함되어 ECR에 있습니다.
                    sh "cp -r recipe/scripts ." // scripts 폴더 복사
                    // sh "cp recipe/build/libs/app.jar ." // 이 라인은 불필요하므로 제거.
                    echo "DEBUG: All artifacts (scripts) copied to Jenkins workspace root." // 로그도 수정
